...
def make_like_tables(cls_cnt, feat_cnts, feat_vals, alpha=1.0):
...
def main():
    ...
    parser.add_argument("--alpha", type=float, default=1.0,
        help="лапласове згладжування (1 = add-one)")
    ...
    cond_x = {"outlook":"Overcast","humidity":"High","wind":"Weak"}

    if not args.no_header:
        print("Outlook=Overcast, Humidity=High, Wind=Weak\n")

    print(f"alpha={args.alpha}")
    probs = get_posterior(cond_x, cls_cnt, like_tbl)

    print(f"P(Yes|x) = {probs.get('Yes',0):.6f}")
    print(f"P(No |x) = {probs.get('No',0):.6f}")
